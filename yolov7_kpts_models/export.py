...
import yolov7_kpts_models
from yolov7_kpts_models.experimental import attempt_load
from yolov7_kpts_utils.activations import Hardswish, SiLU
from yolov7_kpts_utils.general import colorstr, check_img_size, check_requirements, file_size, set_logging
from yolov7_kpts_utils.torch_utils import select_device
...
if __name__ == '__main__':
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, yolov7_kpts_models.common.Conv):  # assign export-friendly activations
            if isinstance(m.act, nn.Hardswish):
                m.act = Hardswish()
            elif isinstance(m.act, nn.SiLU):
                m.act = SiLU()
        # elif isinstance(m, yolov7_kpts_models.yolo.Detect):
    ...
    output_names = None
    if opt.export_nms:
        nms = yolov7_kpts_models.common.NMS(conf=0.01, kpt_label=True)
        nms_export = yolov7_kpts_models.common.NMS_Export(conf=0.01, kpt_label=True)
